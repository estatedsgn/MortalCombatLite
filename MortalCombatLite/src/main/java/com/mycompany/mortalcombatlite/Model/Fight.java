...
public class Fight {
    ...
    Player player;
    Enemy enemy;
    public Place location = new Place();
    public ArrayList<Act> actionsList = new ArrayList<>() {
        {
            add(new Hit());
            add(new Block());
            add(new Debuff());
    ...
    public void checkDeath(ArrayList<Result> results, int locationsNumber, Enemy[] enemiesList) throws IOException {
        ...
        if (player.getHealth() <= 0 | enemy.getHealth() <= 0) {
            if (location.getCurrentLocation() == locationsNumber & "Arseni".equals(enemy.getName())) {
    ...
    public void endRound(Enemy[] enemiesList) throws IOException {
        ...
        if (player.getHealth() > 0) {
            ...
            if ("Arseni".equals(enemy.getName())) {
